...
%       Tf = norm(path(i, 1 : 4) - path(i + 1, 1 : 4))/0.8;
        ...
        in = num2cell([parent_node(1 : 9), node(1 : 9)]);
        Tf = roots(quadf_time(in{:}));
        Tf = Tf(imag(Tf) == 0);
        Tf = Tf(Tf >= 0);
        tf = Tf(1);
        cost = quadf_cost(Tf(1), in{:});
        for i = 2 : size(Tf)
            costnow = quadf_cost(Tf(i), in{:});
            if costnow<cost
                tf = Tf(i);
                cost = costnow;
            end
...
%         Tf = norm(parent_node(1 : 3) - node(1 : 3))/4;

        states = @(t)quadf_state(t, Tf, in{:});
        dt = 0.1;
        Horizon = Tf / dt;
        t = linspace(0,Tf,Horizon);
        state = zeros(dim, length(t));
        for j = 1 : Horizon
            state(:, j) = states(t(j));
        end
