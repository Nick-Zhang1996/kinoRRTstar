      ...
      if (!found) {
        ...
        if (i != *ihi) {
          for (nzcount = 1; nzcount <= ii; nzcount++) {
            ...
            i12 = (*ihi + jj) - 1;
            A_data[atmp_re_tmp] = A_data[i12];
            A_data[i12].re = atmp_re;
            A_data[i12].im = atmp_im;
          }
        }
        ...
        if (j + 1 != *ihi) {
          for (nzcount = 0; nzcount < *ihi; nzcount++) {
            ...
            atmp_im = A_data[nzcount + A_size[0] * j].im;
            i12 = nzcount + A_size[0] * (*ihi - 1);
            A_data[jj] = A_data[i12];
            A_data[i12].re = atmp_re;
            A_data[i12].im = atmp_im;
          }
        }
        ...
        if (!found) {
          ...
          if (j != *ilo) {
            for (nzcount = 0; nzcount < *ihi; nzcount++) {
              ...
              atmp_re = A_data[jj].re;
              atmp_im = A_data[nzcount + A_size[0] * (j - 1)].im;
              i12 = nzcount + A_size[0] * (*ilo - 1);
              A_data[jj] = A_data[i12];
              A_data[i12].re = atmp_re;
              A_data[i12].im = atmp_im;
            }
