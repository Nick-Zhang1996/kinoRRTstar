...
void sqr_eucl_dist(const emxArray_real_T *array, emxArray_real_T *e_dist)
{
  ...
  for (xoffset = 0; xoffset < 3; xoffset++) {
    ...
    for (i0 = 0; i0 <= loop_ub; i0++) {
      sqr_e_dist->data[i0 + sqr_e_dist->size[0] * xoffset] = array->data[i0 +
        array->size[0] * xoffset] * array->data[i0 + array->size[0] * xoffset];
    }
  }
  ...
  if (sqr_e_dist->size[0] == 0) {
    ...
    xoffset = 2 * vstride;
    for (loop_ub = 0; loop_ub < vstride; loop_ub++) {
      e_dist->data[loop_ub] += sqr_e_dist->data[xoffset + loop_ub];
    }
  }
