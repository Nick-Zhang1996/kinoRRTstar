...
double collisionKnowTf(const double parent[8], const double node[8], const
  creal_T *Tf)
{
  ...
  linspace(*Tf, t);
  for (i = 0; i < 10; i++) {
    DI_state(t[i + 1], *Tf, parent[0], parent[1], parent[2], parent[3], node[0],
             node[1], node[2], node[3], *(double (*)[4])&b_state[i << 2]);
  }
