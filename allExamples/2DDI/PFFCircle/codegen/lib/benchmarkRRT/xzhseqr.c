  ...
  if (1 != h_size[0]) {
    ...
    for (i = 2; i <= n; i++) {
      ...
      if (h_data[i6].im != 0.0) {
        ...
        h_data[i6].re = rt_hypotd_snf(h_data[(i + h_size[0] * (i - 2)) - 1].re,
          h_data[(i + h_size[0] * (i - 2)) - 1].im);
        h_data[i6].im = 0.0;
        ix0_tmp = (i - 1) * ldh;
        ix0 = i + ix0_tmp;
        i6 = ix0 + ldh * (n - i);
        for (k = ix0; ldh < 0 ? k >= i6 : k <= i6; k += ldh) {
          ab = h_data[k - 1].re;
          tst = h_data[k - 1].im;
      ...
      while ((!exitg2) && (its < 301)) {
        ...
        while ((!exitg3) && (k + 1 > L + 2)) {
          ...
          if (ba <= SMLNUM) {
            ...
            tst = (fabs(h_data[i6 - 1].re) + fabs(h_data[(k + h_size[0] * (k - 1))
                    - 1].im)) + bb;
            if (tst == 0.0) {
              if (k - 1 >= 1) {
              ...
              for (j = m; j <= i + 1; j++) {
                if (j != m + 1) {
                  if (n > j) {
                    ix0 = j + j * ldh;
                    i6 = ix0 + ldh * ((n - j) - 1);
                    for (u1 = ix0; ldh < 0 ? u1 >= i6 : u1 <= i6; u1 += ldh) {
                      ab = h_data[u1 - 1].re;
                      tst = h_data[u1 - 1].im;
