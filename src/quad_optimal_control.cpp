...
vector<float> QuadOptimalControl::roots(vector<float> coeffs){
  ...
  critical_points = roots(der);
  // evaluate polynomial at each critical point
  // find interval of sign change
  float point = critical_points.begin();
  // true means positive
  bool last_sign = peval(point) > 0;
  float last_point = point;
  point++;
  
  for (point; point<critical_points.end(); point++){
    // find sign change
    if (peval(point) > 0 != last_sign){
      sol.push_back(falsePosition(coeffs, last_point, point));
      last_sign = !last_sign;
      last_point = point;
    }
  }
  ...
  bool sol_at_left  = sign_neg_inf != peval(critical_points.front()) > 0;
  bool sol_at_right = sign_pos_inf != peval(critical_points.front()) > 0;
  ...
  if (sol_at_left || sol_at_right){
    ...
    float a_max_abs = max(abs(a_max), abs(a_min));
    if (sol_at_left){
      float low_bound = - (a_max_abs / abs(coeffs.front()))*10;
      sol.push_front(falsePosition(coeffs, low_bound, point.front());
    }
    if (sol_at_right){
      float high_bound = (a_max_abs / abs(coeffs.front()))*10;
      sol.push_back(falsePosition(coeffs, low_bound, point.front());
    }
  }
...
float QuadOptimalControl::falsePosition(vector<float> coeffs, float low_bound, float high_bound, float err=1e-4){
  ...
  float fa = feval(low_bound, coeffs);
  float fb = feval(high_bound, coeffs);
  assert (fa>0 != fb>0);
  float intercept = (-fa) * (high_bound - low_bound) / (fb - fa) + low_bound;
  if (intercept > 0 == low_bound > 0){
    return falsePosition(intercept, high_bound);
  } else {
    return falsePosition(low_bound, intercept);
  }
...
float QuadOptimalControl::costPartialFinalState(float t_s, float x01, float x02, float x03, float x04, float x05, float x06, float x07, float x08, float x09, float x11, float x12, float x13){
    return (250000*t_s^8 + 9000*t_s^7 + 300*t_s^6*x07^2 + 300*t_s^6*x08^2 + 300*t_s^6*x09^2 + 81*t_s^6 + 1500*t_s^5*x04*x07 + 1500*t_s^5*x05*x08 + 1500*t_s^5*x06*x09 + 9*t_s^5*x07^2 + 9*t_s^5*x08^2 + 9*t_s^5*x09^2 + 1500*t_s^4*x01*x07 + 1500*t_s^4*x02*x08 + 1500*t_s^4*x03*x09 + 2250*t_s^4*x04^2 + 36*t_s^4*x04*x07 + 2250*t_s^4*x05^2 + 36*t_s^4*x05*x08 + 2250*t_s^4*x06^2 + 36*t_s^4*x06*x09 + (81*t_s^4*x07^2)/1000 - 1500*t_s^4*x07*x11 + (81*t_s^4*x08^2)/1000 - 1500*t_s^4*x08*x12 + (81*t_s^4*x09^2)/1000 - 1500*t_s^4*x09*x13 + 4500*t_s^3*x01*x04 + 36*t_s^3*x01*x07 + 4500*t_s^3*x02*x05 + 36*t_s^3*x02*x08 + 4500*t_s^3*x03*x06 + 36*t_s^3*x03*x09 + 36*t_s^3*x04^2 + (81*t_s^3*x04*x07)/250 - 4500*t_s^3*x04*x11 + 36*t_s^3*x05^2 + (81*t_s^3*x05*x08)/250 - 4500*t_s^3*x05*x12 + 36*t_s^3*x06^2 + (81*t_s^3*x06*x09)/250 - 4500*t_s^3*x06*x13 - 36*t_s^3*x07*x11 - 36*t_s^3*x08*x12 - 36*t_s^3*x09*x13 + 2250*t_s^2*x01^2 + 72*t_s^2*x01*x04 + (81*t_s^2*x01*x07)/250 - 4500*t_s^2*x01*x11 + 2250*t_s^2*x02^2 + 72*t_s^2*x02*x05 + (81*t_s^2*x02*x08)/250 - 4500*t_s^2*x02*x12 + 2250*t_s^2*x03^2 + 72*t_s^2*x03*x06 + (81*t_s^2*x03*x09)/250 - 4500*t_s^2*x03*x13 + (81*t_s^2*x04^2)/250 - 72*t_s^2*x04*x11 + (81*t_s^2*x05^2)/250 - 72*t_s^2*x05*x12 + (81*t_s^2*x06^2)/250 - 72*t_s^2*x06*x13 - (81*t_s^2*x07*x11)/250 - (81*t_s^2*x08*x12)/250 - (81*t_s^2*x09*x13)/250 + 2250*t_s^2*x11^2 + 2250*t_s^2*x12^2 + 2250*t_s^2*x13^2 + 36*t_s*x01^2 + (81*t_s*x01*x04)/125 - 72*t_s*x01*x11 + 36*t_s*x02^2 + (81*t_s*x02*x05)/125 - 72*t_s*x02*x12 + 36*t_s*x03^2 + (81*t_s*x03*x06)/125 - 72*t_s*x03*x13 - (81*t_s*x04*x11)/125 - (81*t_s*x05*x12)/125 - (81*t_s*x06*x13)/125 + 36*t_s*x11^2 + 36*t_s*x12^2 + 36*t_s*x13^2 + (81*x01^2)/250 - (81*x01*x11)/125 + (81*x02^2)/250 - (81*x02*x12)/125 + (81*x03^2)/250 - (81*x03*x13)/125 + (81*x11^2)/250 + (81*x12^2)/250 + (81*x13^2)/250)/(t_s^5*(500*t_s + 9)^2);
}
...
float* QuadOptimalControl::interiorPosition(float t_s, float x01, float  x02, float  x03, float  x04, float  x05, float  x06, float  x07, float  x08, float  x09, float  x11, float  x12, float  x13, float  x14, float  x15, float  x16, float  x17, float  x18, float  x19){
  ...
  for (int i=0;i<interior_point_count;i++){
    *(buffer_interior_pos + i * 3 + 0) = x11 + x14*(t - t_s) + (x17*(t - t_s)^2)/2 - ((t - t_s)^5*(12*x01 - 12*x11 + 6*t_s*x04 + 6*t_s*x14 + t_s^2*x07 - t_s^2*x17))/(2*t_s^5) - ((t - t_s)^3*(20*x01 - 20*x11 + 8*t_s*x04 + 12*t_s*x14 + t_s^2*x07 - 3*t_s^2*x17))/(2*t_s^3) - ((t - t_s)^4*(30*x01 - 30*x11 + 14*t_s*x04 + 16*t_s*x14 + 2*t_s^2*x07 - 3*t_s^2*x17))/(2*t_s^4);
    *(buffer_interior_pos + i * 3 + 1) = x12 + x15*(t - t_s) + (x18*(t - t_s)^2)/2 - ((t - t_s)^5*(12*x02 - 12*x12 + 6*t_s*x05 + 6*t_s*x15 + t_s^2*x08 - t_s^2*x18))/(2*t_s^5) - ((t - t_s)^3*(20*x02 - 20*x12 + 8*t_s*x05 + 12*t_s*x15 + t_s^2*x08 - 3*t_s^2*x18))/(2*t_s^3) - ((t - t_s)^4*(30*x02 - 30*x12 + 14*t_s*x05 + 16*t_s*x15 + 2*t_s^2*x08 - 3*t_s^2*x18))/(2*t_s^4);
    *(buffer_interior_pos + i * 3 + 2) = x13 + x16*(t - t_s) + (x19*(t - t_s)^2)/2 - ((t - t_s)^5*(12*x03 - 12*x13 + 6*t_s*x06 + 6*t_s*x16 + t_s^2*x09 - t_s^2*x19))/(2*t_s^5) - ((t - t_s)^3*(20*x03 - 20*x13 + 8*t_s*x06 + 12*t_s*x16 + t_s^2*x09 - 3*t_s^2*x19))/(2*t_s^3) - ((t - t_s)^4*(30*x03 - 30*x13 + 14*t_s*x06 + 16*t_s*x16 + 2*t_s^2*x09 - 3*t_s^2*x19))/(2*t_s^4);
